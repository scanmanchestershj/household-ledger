// Serverless proxy that returns the current Nifty 50 constituent list
...
const CSV_URL = 'https://www.niftyindices.com/IndexConstituent/ind_nifty50list.csv';

// Fallback only used if the live CSV can't be reached — a short list of
// enduring large caps, not a claim of exact, current Nifty 50 membership.
const FALLBACK_LIST = [
  ...
  ['Trent', 'TRENT'], ['Eternal', 'ETERNAL'], ['Adani Green Energy', 'ADANIGREEN']
];
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (list.length < 30) throw new Error('parsed list too short, likely not real CSV content');
    res.status(200).json({ source: 'live', count: list.length, list });
  } catch (e) {
    const list = FALLBACK_LIST.map(([name, symbol]) => ({ name, symbol: symbol + '.NS' }));
    res.status(200).json({ source: 'fallback', count: list.length, list, note: 'Live Nifty 50 list unavailable — showing a fallback set of well-known large caps instead. Reason: ' + (e && e.message ? e.message : 'unknown error') });
  }
};
